...
>
> - `POST /api/parse` (multipart ZIPs) → **HTTP 200** `{problems[], parseErrors[]}` — a **dry run**: parse + group exactly as the import would, but import nothing and touch Polygon not at all. Each problem carries `{slug, name, testsOnly, testCount, languages, languageCodes, hasChecker, hasValidator, hasSolution, extraSolutionCount, hasScoring, groups, archiveCount, timeLimit, memoryLimit}`. `languageCodes` are ISO 639-1 (EN/RU/…) for the parsed languages; `timeLimit`/`memoryLimit` are **always `null`** — the archive format declares no limits (null = archive is silent, never the default). No credentials required. Use it to validate a folder before committing to an import.
> - `POST /api/import-problem` (multipart ZIPs + opts) → **HTTP 202** `{jobId, state, problems[], parseErrors[]}` — returns immediately; the pipeline runs in the background. **Send `timeLimit`/`memoryLimit` explicitly** — omitting them applies the server default *over* the problem's limits (see `docs/maestro/errors.md` → "Time / memory limits").
> - `GET /api/verify-status/{jobId}` → per-problem import state **and live build/verify state**, each with a machine-readable `errorCode` + `clientAction` (`proceed`/`success`/`retry`/`wait`/`halt`). Includes the `IMPORTED_ALREADY_VERIFIED` → `success` inversion. Reports `appliedTimeLimit`/`appliedMemoryLimit` (the TL/ML actually sent to `updateInfo`, so a caller can confirm its limit landed; `null` for tests-only). The per-problem `log[]` **streams live** as the pipeline runs (not only at the end).
