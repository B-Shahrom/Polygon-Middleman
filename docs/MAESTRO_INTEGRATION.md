...
>
> - `POST /api/parse` (multipart ZIPs, optional `MANIFEST.json`) → **HTTP 200** `{problems[], parseErrors[]}` — a **dry run**: parse + group exactly as the import would, but import nothing and touch Polygon not at all. Each problem carries `{slug, name, testsOnly, testCount, languages, languageCodes, hasChecker, hasValidator, hasSolution, extraSolutionCount, hasScoring, groups, archiveCount, timeLimit, memoryLimit, manifest}`. `languageCodes` are ISO 639-1 (EN/RU/…); `timeLimit`/`memoryLimit` are **always `null`** (the archive declares no limits). `manifest` is `null` unless a `MANIFEST.json` describing that slug is uploaded, in which case it is `{timeLimit (ms), memoryLimit (MB), measuredWorstMs, archiveVerified}`. No credentials required.
> - `POST /api/import-problem` (multipart ZIPs + optional `MANIFEST.json` + opts) → **HTTP 202** `{jobId, state, problems[], parseErrors[]}` — returns immediately; the pipeline runs in the background. Limit precedence is **explicit `timeLimit`/`memoryLimit` form field > manifest > server default**; if you send neither a field nor a manifest, the server default is applied *over* the problem's limits. An uploaded `MANIFEST.json` also gates archive integrity (sha256/size mismatch → dropped, reported in `parseErrors` as `integrity: …`). See `docs/maestro/errors.md` → "Time / memory limits".
> - `GET /api/verify-status/{jobId}` → per-problem import state **and live build/verify state**, each with a machine-readable `errorCode` + `clientAction` (`proceed`/`success`/`retry`/`wait`/`halt`). Includes the `IMPORTED_ALREADY_VERIFIED` → `success` inversion. Reports `appliedTimeLimit`/`appliedMemoryLimit` (the TL/ML actually sent to `updateInfo`; `null` for tests-only), `limitsSource` (`form`/`manifest`/`characteristics`/`default`/`mixed`) so a fallback is never mistaken for an explicit value, and `checkerKind` (`standard` when a standard testlib checker was set by name, else `null`). The per-problem `log[]` **streams live** as the pipeline runs (not only at the end).
>
> An optional `characteristics.md` may also travel with the archives (a second authored source): its General-table `TL`/`ML` feed the limit precedence at the `characteristics` tier (below `manifest`), and its `checker` column selects a standard testlib checker by name (no upload) or `custom` (upload the archive's checker.cpp). Maestro keeps sending explicit form fields, so this doesn't change what Maestro relies on — it's primarily for the manual UI.
